...
void main(){	
	...
	vec3 result = vec3(0);
	result += CalcDirLight(lights.dLight, norm, viewDir,color);
	for(int i = 0; i<4; i++){
		result += CalcPointLight(lights.pLight[i], norm, fs_in.v_pos, viewDir,color);
	}
	for(int i=0; i<4; i++){
		result += CalcSpotLight(lights.sLight[i], norm, fs_in.v_pos, viewDir, color);
	}
